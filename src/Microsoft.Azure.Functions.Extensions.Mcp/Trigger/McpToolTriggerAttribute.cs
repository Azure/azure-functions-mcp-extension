// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

using Microsoft.Azure.WebJobs.Description;
using System.Text.Json;

namespace Microsoft.Azure.Functions.Extensions.Mcp;

[AttributeUsage(AttributeTargets.Parameter)]
#pragma warning disable CS0618 // Type or member is obsolete
[Binding(TriggerHandlesReturnValue = true)]
#pragma warning restore CS0618 // Type or member is obsolete
public sealed class McpToolTriggerAttribute(string toolName, string? description) : Attribute
{
    /// <summary>
    /// Gets the name of the MCP tool.
    /// </summary>
    public string ToolName { get; } = toolName;

    /// <summary>
    /// Gets or sets a human-readable description of the MCP tool.
    /// </summary>
    public string? Description { get; set; } = description;

    /// <summary>
    /// Gets or sets the MCP tool properties as a JSON string.
    /// The schema of JSON string must be a valid JSON Schema object with type "object".
    /// When null, the schema is generated from function parameters.
    /// </summary>
    public string? ToolProperties { get; set; }

    /// <summary>
    ///  Gets or sets the MCP tool metadata as a JSON string.
    /// </summary>
    public string? ToolMetadata { get; set; }

    /// <summary>
    /// Gets or sets the MCP tool input schema as a JSON string.
    /// This should only be set when <see cref="UseWorkerInputSchema"/> is true.
    /// The schema of JSON string must be a valid JSON Schema object with type "object".
    /// When null and UseWorkerInputSchema is false, the schema is generated from function parameters.
    /// </summary>
    public string? InputSchema { get; set; }

    /// <summary>
    /// Gets or sets whether the result schema should be provided by the worker instead of being generated by the host extension.
    /// When set to true, the <see cref="ToolResultSchema"/> property must be provided.
    /// When set to false (default), the extension generates the schema from the function return type.
    /// </summary>
    public bool UseResultSchema { get; set; } = false;

    /// <summary>
    /// Gets or sets whether the input schema should be provided by the worker instead of being generated by the host extension.
    /// When set to true, the <see cref="InputSchema"/> property must be provided.
    /// When set to false (default), the extension generates the schema from the function parameters or <see cref="ToolProperties"/>.
    /// </summary>
    public bool UseWorkerInputSchema { get; set; } = false;
}
