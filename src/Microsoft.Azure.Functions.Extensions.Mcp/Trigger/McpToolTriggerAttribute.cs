...
public sealed class McpToolTriggerAttribute(string toolName, string? description) : Attribute
{
    ...
    /// <summary>
    /// Gets or sets the JSON representation of the tool properties schema.
    /// </summary>
    public string? ToolProperties { get; set; }

    /// <summary>
    /// Gets or sets the JSON metadata for the tool.
    /// </summary>
    ...
    /// This should only be set when the worker provides an output schema.
    /// The schema JSON string must be a valid JSON Schema.
    /// </summary>
    public string? OutputSchema { get; set; }

    /// <summary>
    /// Gets or sets whether the worker shapes the tool's result envelope instead of the host extension.
    /// When set to true, the function must return an MCP result type (e.g. <c>McpToolResult</c>); the host
    /// will pass the returned value through as the structured tool result. When set to false (default),
    /// the extension wraps the function's return value into a text-based result itself.
    /// </summary>
    public bool UseResultSchema { get; set; } = false;

    /// <summary>
    /// Gets or sets whether the input schema should be provided by the worker instead of being generated by the host extension.
