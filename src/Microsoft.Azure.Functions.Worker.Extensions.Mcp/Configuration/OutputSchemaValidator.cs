...
/// <summary>
/// Validates that a JSON output schema conforms to the MCP tool output-schema shape
/// (mirrors <see cref="InputSchemaValidator"/>). The root must be a JSON object with
/// <c>"type": "object"</c>.
/// </summary>
internal static class OutputSchemaValidator
{
    /// <summary>
    ...
    public static JsonNode ValidateAndParse(string jsonSchema, string paramName)
    {
        ...
        var node = JsonNode.Parse(jsonSchema)
            ?? throw new ArgumentException("Output schema must be a JSON object.", paramName);
    ...
    public static void Validate(JsonNode schemaNode, string paramName)
    {
        ...
        if (!root.TryGetPropertyValue("type", out var typeNode)
            ...
            || typeStr != "object")
        {
            throw new ArgumentException("Output schema must have a root \"type\" property with value \"object\".", paramName);
        }

        if (root.TryGetPropertyValue("properties", out var propsNode)
            && propsNode is not JsonObject)
        {
            throw new ArgumentException("Output schema \"properties\" must be a JSON object when present.", paramName);
        }

        if (root.TryGetPropertyValue("required", out var requiredNode)
            && requiredNode is not JsonArray)
        {
            throw new ArgumentException("Output schema \"required\" must be a JSON array when present.", paramName);
        }
    }
}
